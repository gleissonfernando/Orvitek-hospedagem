...
import { findUserBotByClientId, findUserBotByHostingAccessKey, updateUserBot } from "../storage/userBotStore";

type HostingShutdownPayload = {
  event?: string;
  eventId?: string;
  sentAt?: string;
  guild?: {
...
type HostingShutdownEventDocument = {
  ...
  payload?: HostingShutdownPayload;
...
type ShutdownResult = {
  ...
  dueAt: string;
  message: string;
  result: "desligado" | "nao_encontrado";
};
...
function describePayload(payload: HostingShutdownPayload): Omit<ShutdownResult, "message" | "result"> {
  return {
...
function logShutdownResult(info: Omit<ShutdownResult, "message" | "result">, result: "desligado" | "nao_encontrado" | "erro", error?: string): void {
...
function isShutdownPayload(payload: HostingShutdownPayload): boolean {
...
export async function shutdownHostingFromPayload(payload: HostingShutdownPayload): Promise<ShutdownResult> {
  ...
  const bot = info.accessKey
    ? await findUserBotByHostingAccessKey(info.accessKey)
    : await findUserBotByClientId(clientId);
